...
from sklearn.metrics import log_loss, roc_auc_score, classification_report, accuracy_score, r2_score
...
class MLP(nn.Module):
    ...
    def evaluate2(self,data):
        ...
        self.to("cpu")
        with torch.no_grad():
            features = data.to("cpu")
            outputs = self(features.float())
            outputs = int(outputs.item())
        return outputs
        ...
        with torch.no_grad():
            for X, y in tqdm(eval_dataloader, desc="evaluating"):
                ...
                if self.task == "cls":
                    ...
                    preds.extend(outputs.squeeze().int().float().tolist())
